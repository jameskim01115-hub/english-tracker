...
# 몇 세미톤 내릴지. 실제 청취 이력(2026-09-05):
#   -1, 포먼트 보존 O  → 톤은 맞는데 어색함
#   -1, 포먼트 보존 X  → 훨씬 자연스러움
#   -1.5, 포먼트 보존 X → 실제 앱(MP3)에서 에코처럼 울림 — 스펙트로그램에서도
#     중간 주파수 대역에 얼룩 확인(R2·R3 엔진 둘 다 같은 문제, 엔진 문제가 아니라
#     이미 합성된 음성을 다시 피치시프트할 때 생기는 부작용으로 보임)
#   -1, 포먼트 보존 X, 실제 MP3 파이프라인 → 얼룩 크게 줄고 에코 없음 → **최종 확정**
# 값을 바꾸면 cache_key() 가 이 값을 키에 넣으므로 이전 캐시와 자동으로 구분된다.
PUCK_LOW_SEMITONES = -1
_kokoro = None
