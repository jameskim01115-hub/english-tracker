...
## 한국어 음성 — 아직 고르는 중 (2026-09-19)

오디오북 「인출 훈련」 모드(한국어 뜻 → 영어)용. 기기 내장 한국어 음성은 Patrick 이
**「너무 기계음이라 거슬린다」**고 해서 서버에서 낸다.

후보 4개를 등록해 뒀다. **아직 채택된 것은 없다** — 배포 후 브라우저에서 하나씩 들어보고
고르면 그 이름을 앱 상수에 박는다. 나머지는 표에 남겨 둔다(옛 선택값 보호).
...
듣는 법 — 주소창에 붙여넣으면 바로 재생된다(`v=` 만 바꿔가며 비교):

```
https://tts.srv1722311.hstgr.cloud/tts?v=KoSunHi&t=오늘 안에 보내주실 수 있나요?
```

- **속도는 서버가 `KO_SPEED`(1.0)로 고정한다.** 앱은 모든 재생에 영어 기준 0.9 를 보내는데
  한국어에 그대로 쓰면 늘어진다. PuckSlow 와 같은 방식이라 어느 화면에서 골라도 같게 나온다.
- 음성 이름은 Microsoft 카탈로그 기준이라 **없는 이름이면 그 음성만 500** 이 난다
  (나머지는 멀쩡). 실제 목록은 `/health` 의 `korean` 과 server.py 주석의 확인 명령 참조.
