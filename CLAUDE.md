...
- **복사 방법이 결과를 바꾼다.** ChatGPT 화면에서 드래그로 복사하면 렌더된 텍스트만 와서
  `**`가 사라지고 일반 강세가 통째로 없어진다 (대문자 피크는 살아남아 정상처럼 보인다).
  **복사 버튼**으로 복사해야 마크다운이 보존된다. 실제로 11건이 이 상태로 들어온 적 있다.
  미리보기가 「리듬맵에 `**볼드`가 하나도 없습니다」 경고로 잡아준다 — 이 경고를 빼지 말 것.
  근본 해결은 ChatGPT에 **Rhythm Map을 코드 블록으로 출력**하게 하는 것이다. 그러면 화면에도
  `**`가 그대로 보여서 보이는 대로 복사된다. 파서가 ```` ``` ````와 백틱 껍데기를 벗겨낸다.
